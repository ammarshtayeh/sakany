...
import { Search, Filter, Compass, X, ChevronDown, Heart } from "lucide-react";
          ...
          <div className="flex gap-4 w-full md:w-auto">
            <div className="relative flex-1 md:w-96 group">
              ...
              <input
                ...
                className="w-full pr-14 pl-6 py-5 rounded-[1.5rem] border border-slate-200 dark:border-slate-800 bg-white dark:bg-card text-slate-900 dark:text-foreground placeholder:text-slate-400 focus:border-primary focus:ring-4 focus:ring-primary/5 outline-none transition-all font-bold text-lg shadow-sm"
              />
            </div>
            <button
              onClick={() => setIsFilterOpen(!isFilterOpen)}
              className={`h-16 w-16 flex items-center justify-center rounded-[1.5rem] border transition-all shadow-xl ${isFilterOpen ? "bg-primary border-primary text-white" : "border-slate-200 dark:border-slate-800 bg-white dark:bg-card text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-850"}`}
            >
              <Filter size={28} />
            </button>
          </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
                        ...
                        <button
                          ...
                          className={`px-5 py-2.5 rounded-xl text-sm font-black transition-all border ${
                            selectedCollege === college.id
                              ...
                              : "bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400 hover:border-primary/30"
                          }`}
                        >
                          {college.name}
                        </button>
                          ...
                          <button
                            ...
                            className={`px-4 py-2.5 rounded-xl text-sm font-black transition-all border flex items-center gap-2 ${
                              isSelected
                                ...
                                : "bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400 hover:border-primary/20"
                            }`}
                          >
                            <span>{feat.name}</span>
                  ...
                  <div className="flex flex-col sm:flex-row items-stretch sm:items-end justify-between gap-4 md:col-span-1">
                    ...
                    <button
                      ...
                      className="flex items-center justify-center gap-2 py-4 px-6 text-slate-400 hover:text-primary transition-colors font-black text-sm uppercase tracking-widest border border-transparent hover:border-slate-200 dark:hover:border-slate-700 rounded-2xl"
                    >
                      <X size={18} />
                      تصفير الفلاتر
                    </button>
