...
export default function StudentsPage() {
  ...
  const listings = useMemo(() => {
    return mockListings
      .filter(l => l.category === 'students' && !l.isPending)
      .filter(l => 
        ...
        <div className="flex flex-col md:flex-row justify-between items-end gap-10 mb-20 animate-in fade-in slide-in-from-bottom-6 duration-700">
          <div className="relative">
            <div className="flex items-center gap-3 text-primary font-black mb-4 group cursor-default">
              ...
              <span className="uppercase tracking-[0.3em] text-sm italic">Explore Nablus Housing</span>
            </div>
            <h1 className="text-5xl md:text-7xl font-black text-white leading-tight">سكن <span className="text-gradient">الشباب</span></h1>
            <p className="text-slate-400 max-w-xl text-lg mt-4 font-bold leading-relaxed">
              أفضل الخيارات المتاحة لطلاب جامعة النجاح الوطنية في نابلس. سكنات موثوقة وقريبة من الحرم الجامعي.
            </p>
          </div>
          
          <div className="flex gap-4 w-full md:w-auto">
            <div className="relative flex-1 md:w-96 group">
              <Search className="absolute right-5 top-1/2 -translate-y-1/2 text-slate-500 group-focus-within:text-primary transition-colors" size={24} />
              <input 
                type="text" 
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                placeholder="ابحث بالحي (رفيديا، تونس...)" 
                className="w-full pr-14 pl-6 py-5 rounded-[1.5rem] border border-white/5 bg-white/5 text-white placeholder:text-slate-600 focus:border-primary/50 focus:bg-white/10 outline-none transition-all font-bold text-lg"
              />
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
          ...
          {listings.length === 0 && (
            <div className="col-span-full py-40 text-center opacity-20 flex flex-col items-center">
              <Compass size={80} className="mb-6 animate-pulse" />
              <p className="text-3xl font-black text-white">لا توجد سكنات معروضة حالياً تطابق بحثك</p>
            </div>
          )}
        </div>
      </main>
