        ...
        <div className="flex flex-col md:flex-row justify-between items-end gap-10 mb-12 animate-in fade-in slide-in-from-bottom-6 duration-700">
          ...
          <div className="flex gap-4 w-full md:w-auto">
            <div className="relative flex-1 md:w-96 group">
              <Search
                className="absolute right-5 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-pink-600 transition-colors"
                size={24}
              />
              <input
                type="text"
                ...
                className="w-full pr-14 pl-6 py-5 rounded-[1.5rem] border border-slate-200 bg-white text-slate-900 placeholder:text-slate-400 focus:border-pink-500 focus:ring-4 focus:ring-pink-500/5 outline-none transition-all font-bold text-lg shadow-sm"
              />
            </div>
            <button
              onClick={() => setIsFilterOpen(!isFilterOpen)}
              className={`h-16 w-16 flex items-center justify-center rounded-[1.5rem] border transition-all shadow-xl ${isFilterOpen ? "bg-pink-600 border-pink-600 text-white" : "border-slate-200 bg-white text-slate-600 hover:bg-slate-50"}`}
            >
              <Filter size={28} />
            </button>
          </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
                        ...
                        <button
                          ...
                          className={`px-6 py-3 rounded-xl text-sm font-black transition-all border ${
                            selectedCollege === college.id
                              ...
                              : "bg-slate-50 border-slate-200 text-slate-500 hover:border-pink-600/30"
                          }`}
                        >
                          {college.name}
                        </button>
                  ...
                  <div className="flex items-end justify-center lg:justify-start">
                    <button
                      ...
                      className="flex items-center gap-2 text-slate-400 hover:text-pink-600 transition-colors font-black text-sm uppercase tracking-widest"
                    >
                      <X size={18} />
                      تصفير الفلاتر
                    </button>
